...
u1=@(t)[(t>=1)*step; 0];     [t1,~,y1]=sim_lin_ode(A,B,C,x0,u1,tFinal);
u2=@(t)[0; (t>=1)*0.2];      [t2,~,y2]=sim_lin_ode(A,B,C,x0,u2,tFinal);
...
u3=@(t)[(t>=1)*step; 0];     [t3,~,y3]=sim_lin_ode(A,B,C,x0,u3,tFinal);
u4=@(t)[0; (t>=1)*step];     [t4,~,y4]=sim_lin_ode(A,B,C,x0,u4,tFinal);
